---
title: "Install OwnPay on Your Server"
sidebarTitle: "Installation"
description: "Complete step-by-step guide to installing OwnPay on shared hosting or a Linux VPS, including Nginx configuration, environment setup, and cron job."
---
...
<Steps>
  ...
  <Step title="Create a MySQL Database and User">
    ...
    <Note>
      Write down the database name, username, password, and host (`localhost` on most setups). You will enter these values during the web installer and optionally into the `.env` file.
    </Note>
  </Step>

  <Step title="Configure the .env File">
    OwnPay reads its core configuration from a `.env` file in the installation root. The web installer writes this file for you, but on a VPS you may prefer to configure it manually before running the installer.
    ...
    APP_KEY=                        # Leave blank — installer generates this automatically
    ...
    A full example with optional Redis configuration:

    ```bash
    APP_NAME="My Payment Gateway"
    APP_URL=https://yourdomain.com
    APP_KEY=
    APP_ENV=production
    APP_DEBUG=false
    ...
    # Optional Redis configuration (see Step 7)
    # CACHE_DRIVER=redis
    # QUEUE_DRIVER=redis
    ...
    <Warning>
      Never commit your `.env` file to version control. It contains database credentials and, after installation, a generated `APP_KEY` that is used for encryption.
    </Warning>
  </Step>
  ...
  <Step title="(Optional) Set Up Redis">
    ...
    Update your `.env` file to use Redis:
    ...
    Alternatively, change these values in **Settings → System** inside the OwnPay admin panel.
  </Step>
</Steps>
