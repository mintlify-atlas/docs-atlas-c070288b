---
title: "OwnPay Code Examples for Common Integration Tasks"
sidebarTitle: "Code Examples"
description: "Ready-to-use PHP and Node.js code examples for creating payments, verifying webhooks, polling status, listing transactions, issuing refunds, and managing customers."
---
...
if ($result['success'] ?? false) {
    ...
    echo "Page {$meta['current_page']} of {$meta['last_page']} ({$meta['total']} total)\n";
}
```

## Issue a Partial Refund

Call `POST /api/v1/transactions/{trx_id}/refunds` to issue a partial or full refund. The refund amount cannot exceed the original transaction's un-refunded balance.
...
$ch = curl_init("$baseUrl/transactions/$trxId/refunds");
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_POST, true);
curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($payload));
curl_setopt($ch, CURLOPT_HTTPHEADER, [
...
if ($httpCode === 201 && ($result['success'] ?? false)) {
    echo 'Refund issued. Refund ID: ' . $result['data']['refund_id'];
} else {
...
Use `POST /api/v1/customers` to create a customer record. Customer PII (name, email, phone) is encrypted at rest using AES-256-GCM. An email hash is stored for deduplication — creating a customer with a duplicate email returns the existing record.
